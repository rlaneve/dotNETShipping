...
namespace dotNETShipping.ShippingProviders
{
	...
	public class FedExProvider : AbstractShippingProvider
	{
		...
		private void parseResponseMessage(string response)
		{
			...
			XmlNodeList nodesEntries = xDoc.SelectNodes("/FDXRateAvailableServicesReply/Entry");
			foreach(XmlNode nodeEntry in nodesEntries)
			{
				...
				if(this._serviceCodes.ContainsKey(rateName))
					service = (AvailableService)this._serviceCodes[rateName];
				else
					continue;
				if(((int)this._services & service.EnumValue) != service.EnumValue)
					continue;
				string rateDesc = this._serviceCodes[rateName].ToString();
				XmlNode rateNode = null;
				if(this.ApplyDiscounts)
					rateNode = nodeEntry.SelectSingleNode("EstimatedCharges/DiscountedCharges/NetCharge");
				else
					rateNode = nodeEntry.SelectSingleNode("EstimatedCharges/ListCharges/NetCharge");
				...
				this._shipment.Rates.Add(new Rate(this.Name, rateName, rateDesc, totalCharges, deliveryDate));
			}
		}

		public System.DateTime getDeliveryDateTime(string serviceCode, System.DateTime deliveryDate)
		{
		...
		private void loadServiceCodes()
		{
			...
			this._serviceCodes.Add("FEDEXGROUND", new AvailableService("Ground Service", 512));
		}
